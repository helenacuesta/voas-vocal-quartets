...
def evaluate_full_individual(test_files, model, patch_len, exp_name, thresh):
    ...
    for song in test_files:
        ...
        # we do not deal with the last frame, so we cut the reference here
        ...
        timestamp, sop = utils.pitch_activations_to_mf0_argmax(pred_s, thresh=thresh[0])
        _, alt = utils.pitch_activations_to_mf0_argmax(pred_a, thresh=thresh[1])
        _, ten = utils.pitch_activations_to_mf0_argmax(pred_t, thresh=thresh[2])
        _, bas = utils.pitch_activations_to_mf0_argmax(pred_b, thresh=thresh[3])
